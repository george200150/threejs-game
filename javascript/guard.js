...
class BasicGuardController {
  ...
  _Init(params) {
    ...
    this._acceleration = new THREE.Vector3(0, 0.5, 100.0);
  ...
  _LoadAnimatedModel() {
    ...
    loader.load(this._params.name, (fbx) => {
      ...
      loader.load('JumpStatic.fbx', (a) => {
        _OnLoad('JumpStatic', a);
      });
    });
  }
  ...
  Update(_, input) {
    if (input._keys.forward || input._keys.backward) {
      ...
      if (input._keys.spacebar) {
        this._parent.SetState('JumpMotion');
      }
      return;
    }
    if (input._keys.spacebar) {
      this._parent.SetState('JumpStatic');
      return;
    }
...
class JumpStaticState extends State {
  constructor(parent) {
    super(parent);
  }
...
class JumpMotionState extends State {
  constructor(parent) {
    super(parent);
  }
...
class GuardFSM extends FiniteStateMachine {
  ...
  _Init() {
    ...
    this._AddState('JumpMotion', JumpMotionState); // TODO: any jump resets to T-pose
    this._AddState('JumpStatic', JumpStaticState);
  }
}
